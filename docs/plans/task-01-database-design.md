...
## 目标
设计并创建系统所需的全部数据库表结构，包括用户、球馆、场地、预约、审计等核心表。
...
**文件:** `backend/src/main/resources/db/migration/V1__init_schema.sql`
...
**验证命令:**
```bash
mysql -u root -p -e "SHOW DATABASES LIKE 'stadium_booking';"
```
...
**文件:** `backend/src/main/resources/db/migration/V2__add_indexes.sql`
...
**验证命令:**
```bash
mysql -u root -p stadium_booking -e "SHOW TABLES;"
```

**预期输出:**
```
+---------------------------+
| Tables_in_stadium_booking |
+---------------------------+
| admin_user                |
| admin_user_role           |
| audit_log                 |
...
git commit -m "feat(db): init database schema for stadium booking system"
```
...
1. **并发安全**: 预约创建时需要使用 `SELECT ... FOR UPDATE` 或唯一索引保证不冲突
2. **软删除**: 主要业务表使用 `deleted_at` 实现软删除
3. **快照字段**: 预约表中保留场馆/场地名称快照，防止改名后历史数据丢失
4. **时区处理**: 所有时间字段使用数据库服务器时区，应用层统一处理
