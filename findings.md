...
- 当前会话默认启用 Planning with Files：复杂任务使用三份持久化 Markdown 文件保存计划、发现与进度
- 新需求：管理端管理员缺少“添加用户”功能，需要先确认这里的“用户”是后台账号还是普通小程序用户
...
- 当前仓库已存在后台账号管理链路：前端路由 `/admin-users`、页面 `admin-web/src/views/admin-user/list.vue`、接口模块 `admin-web/src/api/admin-user.js`、后端控制器 `/admin/admin-users`
- 从命名与既有实现看，现状更像“可查看后台账号并维护球馆绑定”，尚未发现“新增后台账号”入口
- 子代理确认：后台账号管理当前仅支持列表查询与“管理球馆”，不存在 `POST /admin/admin-users`、创建 DTO、创建服务方法，也不存在前端新增弹窗/页面
- 子代理确认：普通用户管理当前仅支持列表/详情/状态切换，不存在 `POST /admin/users` 与新增页面
- 子代理确认：普通用户当前创建路径只有两种：微信首次登录自动创建，或通过数据库预置待绑定用户；因此“后台手工新增普通用户”需要先明确是创建可直接使用的用户档案，还是创建“待绑定”记录
- 子代理确认：普通用户管理当前还存在前后端契约瑕疵（搜索参数名不一致、预约/违约记录返回结构预期不一致），如果本次动到该页面，适合顺手做最小修正
- 用户已确认：普通用户新增采用“预置待绑定”模式
- 用户已确认：后台账号新增只允许创建“场馆管理员”
- 当前实现约束明确：后台账号创建必须同时落 `admin_user`、`admin_user_role(VENUE_STAFF)`、`venue_staff`，否则新账号无法登录或无权限范围
- 当前实现约束明确：普通用户预置创建应保留未绑定语义，不能伪造 `openid`，应创建 `isBound=0` 的待绑定记录
- 当前实现约束明确：普通用户页现有搜索参数与子表数据读取方式和后端契约不一致，本次适合做最小修正避免新增后页面体验继续异常
- 本次已新增后端接口：`POST /admin/admin-users` 用于创建场馆管理员，`POST /admin/users` 用于创建预置待绑定用户
- 本次已新增前端入口：`admin-web/src/views/admin-user/list.vue` 增加“新增场馆管理员”弹窗，`admin-web/src/views/user/list.vue` 增加“新增预置用户”弹窗
...
| 后台账号新增复用现有列表页并使用弹窗表单 | 最小改动接入，避免新增路由与页面状态复杂度 |
| 普通用户新增采用“预置待绑定”记录 | 与现有微信登录后绑定工号/学号链路一致 |
| 用数据库唯一约束兜底 `student_no` | 比纯应用层查重更能避免并发重复数据 |
...
| 新增接口初版缺少强校验与唯一约束兜底 | 经代码审阅后补齐 DTO 校验、密码长度限制和 `student_no` 唯一索引 |
